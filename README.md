...
1. **درخواست داده‌های تاریخچه**:  
   این خط اتصالی کلیدی راه‌اندازی می‌کند که سیگنال `historyBtnClicked` از کلاس `MainWindow` را به اسلات `sendHistoryRequest` از کلاس `Application` متصل می‌کند. هنگامی که در GUI دکمه تاریخچه فشرده می‌شود، این اتصال درخواستی را برای دریافت لاگ تاریخچه از سرور ایجاد می‌کند.

2. **آماده‌سازی ارتباط با سرور**:  
   این خط ارتباطی بین سیگنال `connectBtnClicked` در `MainWindow` و اسلات `connectToServer` در `Application` برقرار می‌کند. این اقدام فرآیند برقراری ارتباط شبکه با سرور را هنگام فشردن دکمه اتصال شروع می‌کند.

3. **نمایش جزئیات کاربر جدید**:  
   در اینجا، سیگنال `newUser` از کلاس `CPSSocket` به اسلات `showUserDetails` در `MainWindow` متصل می‌شود. این تنظیم برای نمایش جزئیات یک کاربر جدید (مانند تگ RFID شناسایی شده، ورود اجازه داده شده/نادیده گرفته شده) به صورت زمان واقعی در GUI در هنگام وقوع این رویدادها استفاده می‌شود.

4. **مدیریت داده‌های تاریخچه جدید**:  
   این اتصال سیگنال `newHistory` از کلاس `CPSSocket` را به اسلات `showHistoryWindow` در `Application` متصل می‌کند. وقتی داده‌های تاریخچه جدید از سرور دریافت می‌شوند، این اقدام باعث می‌شود که برنامه این داده‌ها را در پنجره تاریخچه نمایش دهد.

5. **تغییر المان‌های GUI بر اساس وضعیت اتصال**:  
   این خط اتصالی `connectionChanged` از کلاس `CPSSocket` را به اسلات `changeRightPanelEnabled` در `MainWindow` متصل می‌کند. این اتصال المان‌های GUI را بر اساس وضعیت فعلی اتصال شبکه فعال یا غیرفعال می‌کند (برای مثال، غیرفعال کردن دکمه اتصال برای جلوگیری از اتصال‌های تکراری).
...
این اتصالات برای تعامل و واکنش‌پذیری GUI به تغییرات پشتیبان و ورودی‌های کاربر بسیار اساسی هستند. آنها به سیستم ما امکان می‌دهند که به طور پویا به تغییرات پشتیبان و ورودی‌های کاربر و

اکنش نشان دهد. این اتصالات امکان مدیریت داده‌ها را به طور موثر فراهم می‌کنند و تضمین می‌کنند که رابط کاربری وضعیت و لاگ‌های مهمترین را نشان دهد.

#### منطق نمایش پنجره تاریخچه:
...
## پیاده‌سازی سوکت مشتری

در این بخش جزئیات پیاده‌سازی کلاس `CPSSocket` که تمام ارتباطات TCP/IP بین برنامه‌ی مشتری و سرور را مدیریت می‌کند، ذکر شده است. این کلاس برای ارسال و دریافت داده از شبکه، احراز هویت مدیر و دریافت به‌روزرسانی‌ها درباره رویدادهای ورود و تاریخچه بسیار حیاتی است.

#### `cpssocket.h`

- **کلاس CPSSocket**: از `QObject` به ارث برده شده و ارتباطات سوکت TCP را مدیریت می‌کند.
- **سازنده**: یک سوکت TCP جدید را مقداردهی اولیه می‌کند.
- **getSocket()**: سوکت TCP فعال را برمی‌گرداند.
- **اسلات‌ها**:
  - **connectToServer()**: به سرور با استفاده از مشخصات ورودی متصل می‌شود.
  - **collectingNewData()**: داده‌های جدید از سرور را اداره می‌کند.
- **سیگنال‌ها**:
  - **newUser**: هنگام دریافت داده جدید کاربر، این سیگنال صادر می‌شود.
  - **connectionChanged**: برای به‌روزرسانی اجزای UI بر اساس وضعیت اتصال، این سیگنال صادر می‌شود.
  - **newHistory**: هنگام دریافت داده‌های جدید تاریخچه از سرور، این سیگنال صادر می‌شود.
...
**اتصال به سرور:** 
اتصالی به سرور برقرار کرده و داده‌های احراز هویت را ارسال می‌کند.

**دریافت داده‌های جدید:**
با داده‌های جدید از سرور برخورد می‌کند و آنها را پردازش می‌کند.

**5. استخراج داده کاربر جدید:**
اطلاعات کاربر را از شیء JSON دریافت شده استخراج می‌کند و آنها را صادر می‌کند.
